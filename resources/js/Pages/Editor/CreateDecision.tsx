...
import {
    ...
    FileText, ClipboardCheck, Clock, ArrowRight, CheckCheck, X
} from 'lucide-react';
...
// Define type for form data values
...
export default function CreateDecision({ manuscript, decisionTypes }: Props) {
    const { data, setData, errors, post, processing, reset } = useForm<DecisionFormData>({
        decision: '',
        comments: '',
        revision_deadline: '',
    });
    ...
    // Decision type descriptions for better clarity
    ...
    // Define a consistent mapping for decision labels
    ...
    // Decision icons for visual enhancement
    const decisionIcons = {
    ...
    const handleDecisionChange = (value: string) => {
        ...
        // Show deadline input only for revision decisions
    ...
    const validateForm = () => {
        ...
        if (!data.decision) {
            errors.decision = "Please select a decision type";
        }

        if (showDeadline && !data.revision_deadline) {
            errors.revision_deadline = "Please select a revision deadline";
        }

        if (!data.comments || data.comments.length < 10) {
            errors.comments = "Comments must be at least 10 characters long";
        }
    ...
    const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        console.log('Form submission initiated', { formData: data });
        ...
        if (validationErrors) {
            ...
            // Manually set errors for display
    ...
    const handleSubmit = () => {
        ...
        console.log('Submitting decision data to server', {
            manuscriptId: manuscript.id,
            formData: data,
            url: route('editor.manuscripts.decision', manuscript.id)
        });

        router.post(route('editor.manuscripts.decision', manuscript.id), data, {
            onSuccess: () => {
                ...
                toast({
                    ...
                    description: "The author will be notified about your decision.",
                    variant: "default",
                });
                ...
                <Card className="shadow-lg border-t-4 border-t-primary mb-8">
                    ...
                    <form onSubmit={handleFormSubmit}>
                        <CardContent className="space-y-8 pt-8">
                            ...
                            {/* Decision Summary - shows when all fields are filled */}
            ...
            <AlertDialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
                <AlertDialogContent className="max-w-md">
                    ...
                    <div className="space-y-3 py-1 max-h-[60vh] overflow-y-auto">
                        {/* Manuscript title - with clearer label */}
                        ...
                        {data.decision && (
                            <div className="space-y-3">
                                {/* Decision box - with clear decision text */}
                                <div>
