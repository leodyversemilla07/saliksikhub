...
import { ArrowLeft, Search, UserPlus, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function CreateJournalUser({ availableUsers, roles, filters }: Props) {
    ...
    const handleSearch: FormEventHandler = (e) => {
        e.preventDefault();
        router.get('/admin/journal-users/create', {
        ...
        <AppLayout breadcrumbItems={breadcrumbItems}>
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Assign User to Journal</h1>
                        <p className="text-muted-foreground">
                            Search for an existing user and assign them a role in this journal
                        </p>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Search for users who are not yet assigned to this journal
                                </CardDescription>
                            </CardHeader>
                                        ...
                                        {availableUsers.last_page > 1 && (
                                            <div className="mt-4">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-sm text-muted-foreground">
                                                        Page {availableUsers.current_page} of {availableUsers.last_page}
                                                    </p>
                                                    <div className="flex items-center gap-2">
                                                        {availableUsers.links.map((link, index) => {
                                                            ...
                                                            if (index === availableUsers.links.length - 1) {
                                                                return (
                                                                    <Button
                                                                        key="next"
                                                                        variant="outline"
                            ...
                            <CardContent>
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    {selectedUser ? (
                                        <div className="rounded-lg border p-4 space-y-1">
                                            ...
                                            {selectedUser.affiliation && (
                                                <p className="text-xs text-muted-foreground">{selectedUser.affiliation}</p>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="rounded-lg border border-dashed p-4 text-center">
                                            <p className="text-sm text-muted-foreground">
                                                No user selected. Search and select a user from the list.
                                            </p>
                                        </div>
                                    )}
                                    {errors.user_id && (
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium">Journal Role *</label>
                                        <Select
                                            value={data.role}
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        ...
                                        <Button
                                            type="submit"
                                            disabled={processing || !selectedUser || !data.role}
                                            className="flex-1"
                                        >
