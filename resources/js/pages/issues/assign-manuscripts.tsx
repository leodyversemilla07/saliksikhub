...
export default function AssignManuscripts({
    issue,
    availableManuscripts,
}: AssignManuscriptsProps) {
    ...
    return (
        <AppLayout breadcrumbItems={breadcrumbItems}>
            <Head
                title={`Assign Manuscripts - Vol. ${issue.volume_number}, Issue ${issue.issue_number}`}
            />
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                Select manuscripts that are ready for
                                publication to assign to this issue.
                            </p>
                        </CardHeader>
                        <CardContent>
                            {availableManuscripts.length > 0 ? (
                                ...
                                >
                                    <div className="space-y-4">
                                        {availableManuscripts.map(
                                            (manuscript) => (
                                                <div
                                                    key={manuscript.id}
                                                    className="flex items-start gap-4 rounded-lg border border-gray-200 p-4 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
                                                >
                                                    <Checkbox
                                                        id={`manuscript-${manuscript.id}`}
                                                        checked={selectedManuscripts.includes(
                                                            manuscript.id,
                                                        )}
                                                        onCheckedChange={() =>
                                                            handleManuscriptToggle(
                                                                manuscript.id,
                                                            )
                                                        }
                                                        className="mt-1"
                                    ...
                                    <div className="flex items-center justify-between border-t border-gray-200 pt-6 dark:border-gray-700">
                                        <div className="text-sm text-gray-600 dark:text-gray-400">
                                            {selectedManuscripts.length}{' '}
                                            manuscript(s) selected
                                        </div>
                                        <div className="flex gap-4">
                                            ...
                                            >
                                                {processing
                                                    ? 'Assigning...'
                                                    : `Assign ${selectedManuscripts.length} Manuscript(s)`}
                                            </Button>
                                        </div>
                                    </div>
                                </form>
                                ...
                                <div className="py-12 text-center">
                                    ...
                                    <p className="mb-4 text-gray-600 dark:text-gray-400">
                                        There are no manuscripts ready for
                                        publication that can be assigned to this
                                        issue.
                                    </p>
                                    <Button
                                        variant="outline"
